...
1. 重启 KOreader（加载新代码 + 触发 v5→v6 迁移）
2. 重试昨晚失败的 199 字选段"深入解释"问题：
   - 预期：屏幕显示"正在思考…"并保持（HIGH-1 修复验证点）
   - 预期：10-40 秒后正常收到回答（不再 wantread / 回复为空）
3. crash.log 确认：`migrated settings v5→v6: ai_max_tokens 1024 → 4096`、`max_tokens=4096`
4. 然后继续 Phase 3.D / 3.E / 4 / 5（见 2026-08-14 文档"明日计划"）
